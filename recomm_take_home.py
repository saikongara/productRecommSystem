...
# MAGIC %md 
# MAGIC I have made use of **Databricks Community Edition** to utilize `Spark 2.4.4` and `Python 3.7` to build the recommendation model. The data is loaded into DBFS (Databricks File System) and used as `/FileStore/tables/file_name`
# MAGIC 
# MAGIC Python Libraries used : `functions`,`window`,`requests`, `json` and `bigquery`.

# COMMAND ----------

# MAGIC %md ### Importing required modules
...
# MAGIC %md ### Loading the `Products` dataset provided
...
# MAGIC %md ### Loading the `MCH categories` dataset provided
...
# MAGIC %md ### Loading the `Transactions` dataset provided
...
# MAGIC %md ##### Limiting the number of transaction from the dataset to `10000` as the compute is not sufficient for processing
...
# MAGIC %md ### What are the item ID and name of the top 5 co-purchased items for item with ID 20592676_EA? What about 20801754003_C15?
...
# MAGIC %md ### How would you deploy and serve the model?
# MAGIC 
# MAGIC 1. Assuming The data sources(3) would be designed as tables in database and this model connects to database using JDBC/ODBC connections. Once the testing of this model was successful, the code can be deployed onto Production (Python 3.7 and Spark 2.4.4) and triggers whenever the schedules are in place.
# MAGIC 2. Assuming the the data sources would be real-time, let us consider as kafka events, the code needs a bit more tweaks using Dstreams and StructuralStreams in Spark and the events gets processed real-time. In this scenario code needs to be deployed onto production upon successful completion of testing and triggered once to be executed contineously.

# COMMAND ----------

# MAGIC %md ### POST request submission to **Test Service** : `https://ld-ds-take-home-test.appspot.com/submissions`
